...
export async function POST(request: Request) {
  const payload = waitlistSchema.safeParse(await request.json().catch(() => ({})));
  ...
  if (!supabase) {
    return NextResponse.json(
      {
        message:
          "Supabase is not configured yet. Add SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY to enable submissions.",
      },
      { status: 503 },
    );
  }
  ...
  return NextResponse.json({ message: "You're on the update list." });
}
